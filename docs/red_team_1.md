...
- R3 regime-block CIs: **not robust.** SAR CI includes zero in ALL cuts — full +0.25 [−0.22, +0.65], drop-2008 +0.08 [−0.32, +0.56], drop-2020 +0.23 [−0.21, +0.67], drop-2026 +0.16 [−0.28, +0.51], drop-all-three +0.07 [−0.43, +0.41], winsorized-5% +0.21 [−0.17, +0.54]. Raw \|CAR\| significant only with 2008 in (drop-2008 → +3.66 [−0.16, +8.49] crosses zero; drop-all-three → +3.41 [−1.49, +6.48]). A 2008/GFC-regime effect, not a general ripple. Receipt: `data/h1_regimeblock.json`.
- R4 subset re-runs: SAR — full +0.25 [−0.22, +0.65]; ex-endogenous +0.23 [−0.24, +0.69]; **geopolitical-only +0.54 [−0.020, +1.022]** (borderline, just includes zero). Raw — full +5.56, ex-endo +5.17, geo-only +6.91 [1.62, 10.56]. Endogenous drop barely moves it (attack 4 not the driver); geopolitical-only is *stronger* and near-significant even under SAR. **Flag list (25 events) AWAITING JOE** — mechanical rule over-flags food/metal/COVID policy events that coincide with the 2020 oil crash but don't react to oil. Receipts: `data/h1_subsets.json`, `data/endogenous_flags.json`.
