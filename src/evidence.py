...
Claims covered: the registered H1 edge, the validated cross-asset ripple nodes, and the battery's
validated conditioned edges (copper_growth, hy_credit_stress, ...). Every one is a CLAIM (tier
validated); nulls are reported elsewhere, not here.
...
def _write_index(packs):
    L = ["# EVIDENCE — every claim, one hop from its receipt", "",
         ...
         "Regenerated by `src/evidence.py`. Only **validated** claims get a pack; nulls are reported in "
         "`EDGE_PORTFOLIO.md` / `evaluation.json`.", "",
