...
RUN_OBJECTS = [
    ...
    # The registered explanatory ablation. It holds the field-use audit, which is where the
    # README's and paper's comparison counts (41,997 target-candidate comparisons, 20,915
    # market-only) come from; without it those numbers read as untraceable.
    {"path": "data/structural_surface/ablation/summary.json", "run_key": None,
     "stamp_key": None, "role": "the registered component and concentration ablation"},
...
def render_markdown(inv):
    ...
    total = sum(c.values())
    lines = [
        # The status banner is emitted here rather than added by hand, because this file is
        # regenerated: a hand-added banner would silently disappear on the next run and
        # src/doc_status_guard.py would then fail mid-release-gate.
        "> **ARCHIVED — HISTORICAL RECORD, NOT A CURRENT CLAIM.** A generated inventory of the "
        "legacy engine's published numbers. Preserved for audit; current release status is in "
