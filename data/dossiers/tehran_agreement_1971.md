...
(a) two independent registrable domains — `history.state.gov` (S1, S2, primary) and `ora.ox.ac.uk` (S3, scholarly
secondary); (b)–(f) were already met.
**Corroboration strength, stated:** S3 is retrospective (2011) and attests the Tehran negotiation, its outcome in
kind (a collective rise in the posted price and the tax rate) and its causal origin in the Libyan settlement. It
carries **no figure**: the 30¢/50¢ per barrel and the $1.4bn/$12bn totals rest on S1 alone and are projections
made in March 1971, not measurements.

*Superseded status, 2026-09-03, kept as the record: "partial — fails (a). Two primary documents, one registrable
domain. This is the OPEC-sourcing failure SPINE_REGISTRATION Amendment 2 registered in advance." The failure was
real for the routes then tested and is no longer the outcome; `ora.ox.ac.uk` was not tested in the first pass.*
